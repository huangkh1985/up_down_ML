"""
MA10多时间窗口预测模型训练脚本
为1天、3天、5天、10天分别训练独立的MA10状态预测模型
"""
...
def create_ma10_target(stock_data, forecast_horizon):
    """
    创建MA10状态标签
    ...
    --------
    target: Series
        1表示未来N天收盘价>=MA10（强势），0表示<MA10（弱势）
    """
    target = []
    ...
    for i in range(len(stock_data) - forecast_horizon):
        ...
        future_ma10 = stock_data['MA10'].iloc[future_idx]
        
        # 判断未来收盘价是否>=MA10
        if pd.notna(future_close) and pd.notna(future_ma10) and future_ma10 > 0:
            label = 1 if future_close >= future_ma10 else 0
            target.append(label)
        else:
...
def prepare_training_data(all_stock_data, forecast_horizon):
    ...
    """
    print(f"\n{'='*80}")
    print(f"[数据准备] 准备 {forecast_horizon} 天预测的训练数据（MA10）...")
    ...
    for stock_code, stock_data in all_stock_data.items():
        try:
            ...
            # 确保有MA10列
            if 'MA10' not in stock_data.columns or 'Close' not in stock_data.columns:
                print("[跳过] 缺少MA10或Close列")
                continue
            ...
            # 创建标签
            target = create_ma10_target(stock_data, forecast_horizon)
    ...
    print(f"\n{'='*80}")
    print(f"[完成] {forecast_horizon}天数据准备完成（MA10）:")
    print(f"   总样本数: {len(X)}")
...
def train_model(X, y, forecast_horizon):
    ...
    """
    print(f"\n{'='*80}")
    print(f"[模型训练] 训练 {forecast_horizon} 天预测模型（MA10）...")
...
def main():
    """主函数"""
    print("\n" + "="*80)
    print("MA10多时间窗口模型训练系统")
    print("="*80)
    print(f"训练时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"训练目标: 为1天、3天、5天、10天分别训练独立的MA10预测模型")
    print("="*80)
    ...
    if trained_models:
        ...
        save_path = 'models/ma10_multi_horizon_models.pkl'
