    ...
    try:
        ...
        for horizon, model_info in models.items():
            ...
            if 'precision' in model_info:
                print(f"  精确率: {model_info['precision']:.4f}")
            if 'recall' in model_info:
                print(f"  召回率: {model_info['recall']:.4f}")
            
            # 测试预测功能
            try:
                model = model_info['model']
                n_features = len(feature_columns) if feature_columns else 75  # 默认特征数
                dummy_input = np.random.randn(1, n_features)
...
if __name__ == "__main__":
    ...
    print(f"形态识别模型: {'✓ 通过' if pattern_ok else '✗ 失败'}")
    print()
    
    if ma20_ok and data_ok:
        print("✓ 核心模型验证通过，可以使用Streamlit应用！")
        if pattern_ok:
            print("✓ 形态识别模型也通过，完整功能可用！")
        print()
        print("运行应用:")
        print("  streamlit run app_streamlit.py")
        print()
        print("测试建议:")
        print("  1. 测试MA20预测标签页（使用新的4个独立模型）")
        print("  2. 测试形态信号预测标签页")
        print("  3. 对比两种方法的预测结果")
        sys.exit(0)
    else:
